...
class MyStackedConvBlocks(nn.Module):
    def __init__(self,
                 num_convs: int,  # 2
                 conv_op: Type[_ConvNd],  # nn.Conv3d
                 input_channels: int,
                 output_channels: Union[int, List[int], Tuple[int, ...]],  # 32 * 2^i
                 kernel_size: Union[int, List[int], Tuple[int, ...]],
                 initial_stride: Union[int, List[int], Tuple[int, ...]],
                 conv_bias: bool = False,
                 norm_op: Union[None, Type[nn.Module]] = None,
        ...
        # TODO 改 convs，加一个先upsample（transpconvs）再卷积再downsample（pooling）的分支
        stride = [2, 2, 2]
        self.up_down_convs = nn.Sequential(
            nn.ConvTranspose3d(
                input_channels,
                input_channels,
                stride=stride,
            ...
            # TODO 下采样
            ConvDropoutNormReLU(
                conv_op, output_channels[-1], output_channels[-1], kernel_size, stride, conv_bias, norm_op,
                norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs, nonlin_first
            ),
            # nn.Conv3d(output_channels[-1],output_channels[-1],kernel_size,stride,bias=conv_bias)
        )
        # TODO 两个分支合并
        self.concat = nn.Conv3d(in_channels=output_channels[-1] * 2, out_channels=output_channels[-1], kernel_size=[1, 1, 1])
    ...
    def forward(self, x):
        x1 = self.convs(x)
        x2 = self.up_down_convs(x)
        assert x1.shape == x2.shape
        # TODO concat并合并成一个
        # res=self.concat(torch.concat([x1, x2], dim=1))
        # assert x1.shape==res.shape
        return self.concat(torch.concat([x1,x2],dim=1))
