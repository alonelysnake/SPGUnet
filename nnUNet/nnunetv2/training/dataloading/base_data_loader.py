...
class nnUNetDataLoaderBase(DataLoader):
    ...
    def get_bbox(self, data_shape: np.ndarray, force_fg: bool, class_locations: Union[dict, None],
        ...
        lbs = [- need_to_pad[i] // 2 for i in range(dim)]# initial->final patch size会做一次crop，故留出padding? 确定下限取值的下限
        ubs = [data_shape[i] + need_to_pad[i] // 2 + need_to_pad[i] % 2 - self.patch_size[i] for i in range(dim)]# 确定下限取值的上限
